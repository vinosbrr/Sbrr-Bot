...
async function GroupUpdate(vino, m, store) {
	if (!m.messageStubType || !m.isGroup) return
	if (global.db?.groups[m.chat]?.setinfo && vino.public) {
		...
		if (messages[m.messageStubType]) {
			await vino.sendMessage(m.chat, { text: `${admin} ${messages[m.messageStubType]}`, mentions: [m.sender, ...(m.messageStubParameters[0]?.includes('@') ? [`${m.messageStubParameters[0]}`] : [])]}, { ephemeralExpiration: m.expiration || store?.messages[m.chat]?.array?.slice(-1)[0]?.metadata?.ephemeralDuration || 0 })
		} else {
			console.log({
...
async function GroupCacheUpdate(vino, update, store, groupCache) {
	try {
		for (let n of update) {
...
async function GroupParticipantsUpdate(vino, { id, participants, author, action }, store, groupCache) {
	try {
		...
		if (global.db?.groups[id] && store?.groupMetadata[id]) {
			...
			for (let n of participants) {
				let profile;
				try {
					profile = await vino.profilePictureUrl(n, 'image');
				} catch {
				...
				if (messageText && vino.public) {
					await vino.sendMessage(id, {
...
async function LoadDataBase(vino, m) {
	try {
...
async function MessagesUpsert(vino, message, store, groupCache) {
	try {
		let botNumber = await vino.decodeJid(vino.user.id);
		const msg = message.messages[0];
		if (!store.groupMetadata || Object.keys(store.groupMetadata).length === 0) {
			store.groupMetadata ??= await vino.groupFetchAllParticipating().catch(e => ({}))
		}
		...
		const m = await Serialize(vino, msg, store, groupCache)
		require('../vino')(vino, m, msg, store, groupCache);
		if (type === 'interactiveResponseMessage' && m.quoted && m.quoted.fromMe) {
			await vino.appendResponseMessage(m, JSON.parse(m.msg.nativeFlowResponseMessage.paramsJson).id);
		}
		if (global.db?.set[botNumber] && global.db?.set[botNumber]?.readsw) {
			if (msg.key.remoteJid === 'status@broadcast') {
				await vino.readMessages([msg.key]);
				if (/protocolMessage/i.test(type)) vino.sendFromOwner(global.owner, 'Status dari @' + msg.key.participant.split('@')[0] + ' Telah dihapus', msg, { mentions: [msg.key.participant] });
				if (/(audioMessage|imageMessage|videoMessage|extendedTextMessage)/i.test(type)) {
					...
					await vino.sendFromOwner(global.owner, `Melihat story dari @${msg.key.participant.split('@')[0]}\n${keke}`, msg, { mentions: [msg.key.participant] });
				}
			}
		}
	} catch (e) {
...
async function Solving(vino, store) {
	vino.serializeM = (m) => MessagesUpsert(vino, m, store)
	...
	vino.getName = (jid, withoutContact  = false) => {
		const id = vino.decodeJid(jid);
		if (id.endsWith('@g.us')) {
			const groupInfo = store.contacts[id] || vino.groupMetadata(id) || {};
	...
	vino.sendContact = async (jid, kon, quoted = '', opts = {}) => {
		let list = []
		for (let i of kon) {
			list.push({
				displayName: await vino.getName(i + '@s.whatsapp.net'),
				vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await vino.getName(i + '@s.whatsapp.net')}\nFN:${await vino.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.ADR:;;Indonesia;;;;\nitem2.X-ABLabel:Region\nEND:VCARD` //vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await vino.getName(i + '@s.whatsapp.net')}\nFN:${await vino.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:whatsapp@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/vino_dev\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
			})
		}
		vino.sendMessage(jid, { contacts: { displayName: `${list.length} Kontak`, contacts: list }, ...opts }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	vino.profilePictureUrl = async (jid, type = 'image', timeoutMs) => {
		const result = await vino.query({
			tag: 'iq',
			attrs: {
	...
	vino.sendPoll = (jid, name = '', values = [], quoted, selectableCount = 1) => {
		return vino.sendMessage(jid, { poll: { name, values, selectableCount }}, { quoted, ephemeralExpiration: quoted.expiration || 0 })
	}
	
	vino.sendFileUrl = async (jid, url, caption, quoted, options = {}) => {
		async function getFileUrl(res, mime) {
			if (mime && mime.includes('gif')) {
				return vino.sendMessage(jid, { video: res.data, caption: caption, gifPlayback: true, ...options }, { quoted });
			} else if (mime && mime === 'application/pdf') {
				return vino.sendMessage(jid, { document: res.data, mimetype: 'application/pdf', caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('image')) {
				return vino.sendMessage(jid, { image: res.data, caption: caption, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('video')) {
				return vino.sendMessage(jid, { video: res.data, caption: caption, mimetype: 'video/mp4', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			} else if (mime && mime.includes('webp') && !/.jpg|.jpeg|.png/.test(url)) {
				return vino.sendAsSticker(jid, res.data, quoted, options);
			} else if (mime && mime.includes('audio')) {
				return vino.sendMessage(jid, { audio: res.data, mimetype: 'audio/mpeg', ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
			}
		}
	...
	vino.sendGroupInvite = async (jid, participant, inviteCode, inviteExpiration, groupName = 'Unknown Subject', caption = 'Invitation to join my WhatsApp group', jpegThumbnail = null, options = {}) => {
		...
		const invite = await vino.relayMessage(participant, message.message, { messageId: message.key.id })
		return invite
	}
	
	vino.sendFromOwner = async (jid, text, quoted, options = {}) => {
		for (const a of jid) {
			await vino.sendMessage(a.replace(/[^0-9]/g, '') + '@s.whatsapp.net', { text, ...options }, { quoted });
		}
	}
	
	vino.sendTextMentions = async (jid, text, quoted, options = {}) => vino.sendMessage(jid, { text: text, mentions: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net'), ...options }, { quoted })
	
	vino.sendAsSticker = async (jid, path, quoted, options = {}) => {
		...
		return vino.sendMessage(jid, { sticker: { url: result }, ...options }, { quoted, ephemeralExpiration: quoted.expiration || 0 });
	}
	
	vino.downloadMediaMessage = async (message) => {
	...
	vino.downloadAndSaveMediaMessage = async (message, filename, attachExtension = true) => {
		const buffer = await vino.downloadMediaMessage(message);
	...
	vino.appendResponseMessage = async (m, text) => {
		let apb = await generateWAMessage(m.chat, { text, mentions: m.mentionedJid }, { userJid: vino.user.id, quoted: m.quoted });
		apb.key = m.key
		apb.key.fromMe = areJidsSameUser(m.sender, vino.user.id);
		if (m.isGroup) apb.participant = m.sender;
		vino.ev.emit('messages.upsert', {
			...m,
	...
	vino.sendMedia = async (jid, path, fileName = '', caption = '', quoted = '', options = {}) => {
		const { mime, data, filename } = await vino.getFile(path, true);
		...
		let anu = await vino.sendMessage(jid, { [type]: { url: pathFile }, caption, mimetype, fileName, ...options }, { quoted, ...options });
		await fs.unlinkSync(pathFile);
		return anu;
	}
	
	vino.sendListMsg = async (jid, content = {}, options = {}) => {
		...
		const hasil = await vino.relayMessage(msg.key.remoteJid, msg.message, {
	...
	vino.sendButtonMsg = async (jid, content = {}, options = {}) => {
	...
	vino.newsletterMsg = async (key, content = {}, timeout = 5000) => {
		...
		if (react) {
			...
			const msg = await generateWAMessageContent(media, { upload: vino.waUploadToServer });
			const anu = await vino.query({
				tag: 'message',
	...
	vino.sendCarouselMsg = async (jid, body = '', footer = '', cards = [], options = {}) => {
		async function getImageMsg(url) {
			const { imageMessage } = await generateWAMessageContent({ image: { url } }, { upload: vino.waUploadToServer });
			return imageMessage;
		}
		...
		const hasil = await vino.relayMessage(msg.key.remoteJid, msg.message, { messageId: msg.key.id });
		return hasil
	}
	
	if (vino.user && vino.user.id) {
		const botNumber = vino.decodeJid(vino.user.id);
		if (global.db?.set[botNumber]) {
			vino.public = global.db.set[botNumber].public
		} else vino.public = true
	} else vino.public = true
...
async function Serialize(vino, m, store, groupCache) {
	const botNumber = vino.decodeJid(vino.user.id)
	if (!m) return m
	...
	if (m.key) {
		...
		m.sender = vino.decodeJid(m.fromMe && vino.user.id || m.participant || m.key.participant || m.chat || '')
		if (m.isGroup) {
			if (!store.groupMetadata) store.groupMetadata = await vino.groupFetchAllParticipating().catch(e => ({}));
			let metadata = store.groupMetadata[m.chat] ? store.groupMetadata[m.chat] : (store.groupMetadata[m.chat] = groupCache.get(m.chat))
			if (!metadata) {
				metadata = await vino.groupMetadata(m.chat).catch(e => ({}))
	...
	if (m.message) {
		...
		if (m.quoted) {
			...
			m.quoted.sender = vino.decodeJid(m.msg.contextInfo.participant)
			m.quoted.fromMe = m.quoted.sender === vino.decodeJid(vino.user.id)
			...
			m.getQuotedObj = async () => {
				...
				let q = await store.loadMessage(m.chat, m.quoted.id, vino)
				return await Serialize(vino, q, store, groupCache)
			}
			m.quoted.key = {
				remoteJid: m.msg?.contextInfo?.remoteJid || m.chat,
				participant: m.quoted.sender,
				fromMe: areJidsSameUser(vino.decodeJid(m.msg?.contextInfo?.participant), vino.decodeJid(vino?.user?.id)),
			...
			m.quoted.download = () => vino.downloadMediaMessage(m.quoted)
			m.quoted.delete = () => {
				vino.sendMessage(m.quoted.chat, {
					delete: {
						remoteJid: m.quoted.chat,
	...
	m.download = () => vino.downloadMediaMessage(m)
	
	m.copy = () => Serialize(vino, proto.WebMessageInfo.fromObject(proto.WebMessageInfo.toObject(m)))
	...
	m.reply = async (content, options = {}) => {
		...
		if (typeof content === 'object') {
			return vino.sendMessage(chat, content, { ...options, quoted, ephemeralExpiration })
			...
			try {
				if (/^https?:\/\//.test(content)) {
					...
					if (/gif|image|video|audio|pdf|stream/i.test(mime)) {
						return vino.sendMedia(chat, data.data, '', caption, quoted, content)
					} else {
						return vino.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
					}
				} else {
					return vino.sendMessage(chat, { text: content, mentions, ...options }, { quoted, ephemeralExpiration })
				}
